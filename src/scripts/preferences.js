...
function exportPretty() {
    ...
    dialog.showSaveDialog({
        ...
        for (var r=1;r<=puzzles[currentPuzzle].sessions[currentSession].records.length;r++) {
            str+="\n"+(r)+" "+sessionRecords.rows[r].children[1].children[0].innerHTML+" "+sessionRecords.rows[r].children[2].children[0].innerHTML+" "+sessionRecords.rows[r].children[3].children[0].innerHTML   
        }
        fs.writeFile(fileName, str, function (err) {
            if (err) {
                alert("An error ocurred creating the file: "+ err.message)
...
function clipboardPretty() {
    ...
    for (var r=1;r<=sessionStatsTable.rows.length-1;r++) {
        str+="\nBest "+sessionStatsTable.rows[r].children[0].children[0].innerHTML+": "+sessionStatsTable.rows[r].children[2].children[0].innerHTML 
    }
